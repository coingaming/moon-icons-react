...
const { identity, filter, pipe, prop, head, map, replace, hasPath } = require('ramda');

const filterByPage = (pageFilter) => (data) => {
  const pagesList = data.shortcuts.pages;
  if (typeof pageFilter === 'string') {
    return filter((page) => {
      return page?.name.includes(pageFilter)
    })(pagesList);
  }
...
const rename = (name) => {
  ...
  // remove ", Type=Stroke" 'Default=', 'Property 1=' parts
  const trimmedArr = ['Default=', 'Property 1=', 'Symbol=', 'Logotype=', "Teams=", "Types="];
  const headName = head(nameArr)
  const waste = trimmedArr.find(el => headName.includes(el));
  const trimmedName = headName.replace(waste, '');
